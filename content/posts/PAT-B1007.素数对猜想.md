...
那我们先来大概说一下解题的思路。我们都知道，素数是从 2 开始的，而第一个素数 2 和 第二个素数 3 之间相差 1，所以我们在用循环遍历查找符合条件的素数对的时候就可以从 3 开始遍历；C++ 这次用了函数去解比较简单，先定义一个判断是否为素数的函数，是素数返回 true，不是返回 false；然后再主函数中，直接遍历出相差 2 的两个素数求解，得出一个结果就计数加一；而 Java 这次没有用到函数去解，而是定义了一个素数数组，把遍历出来的素数存入素数数组中，用遍历到的素数与数组中的素数错位相减等于 2 ，符合条件的 `count ++` 。
...
C++ 解法用到函数去解，整体分为两部分，先把函数写出，然后在主函数中去调用，方便快捷高效。

函数这次用到 Bool 函数，没有返回值，而是返回 true 和 false；写 bool 函数还是套用判断素数的公式，循环从 2 开始遍历，以不超过数本身的平方为循环界限，用需要判定的数对 i 进行取余操作，能被整除的就不是素数，反之为素数；bool 函数为：
